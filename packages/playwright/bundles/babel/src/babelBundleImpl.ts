...
function babelTransformOptions(isTypeScript: boolean, isModule: boolean, pluginsPrologue: [string, any?][], pluginsEpilogue: [string, any?][], jsxImportSource?: string): TransformOptions {
  ...
  plugins.push([require('@babel/plugin-transform-react-jsx'), {
    ...
    runtime: 'automatic',
    importSource: jsxImportSource ?? path.dirname(require.resolve('playwright')),
  }]);
...
export function babelTransform(code: string, filename: string, isModule: boolean, pluginsPrologue: [string, any?][], pluginsEpilogue: [string, any?][], inputSourceMap?: EncodedSourceMap, jsxOptions?: { jsxImportSource?: string }): BabelFileResult | null {
  if (isTransforming)
    return null;
  ...
  isTransforming = true;
  try {
    const options = babelTransformOptions(isTypeScript(filename), isModule, pluginsPrologue, pluginsEpilogue, jsxOptions?.jsxImportSource);
    if (inputSourceMap) {
