...
class Registry {
  ...
  async start(packages: { [pkg: string]: string }) {
    ...
    await Promise.all(Object.entries(packages).map(([pkg, tar]) => this._publish(pkg, tar)));

    this._server = http.createServer(async (req, res) => {
      if (req.method !== 'GET') return res.writeHead(405).end();

      const url = new URL(req.url, kPublicNpmRegistry);
      let [/* empty */, userSuppliedPackageName, /* empty */, userSuppliedFilename] = url.pathname.split('/');
  ...
  private async _publish(pkg: string, tarPath: string) {
...
export const test = _test.extend<{
    tmpWorkspace: string,
    exec: (cmd: string, args: string[]) => Promise<SpawnResult>
    npm: (...args: string[]) => Promise<SpawnResult>,
    npx: (...args: string[]) => Promise<SpawnResult>,
    registry: Registry,
        }>({
          ...
          registry: async ({}, use, testInfo) => {
            ...
            await registry.start(JSON.parse((await fs.promises.readFile('./registry.json')).toString()));
            await use(registry);
            await registry.shutdown();
          },
          exec: async ({ registry, tmpWorkspace }, use, testInfo) => {
            await use((cmd: string, args: string[]) => spawnAsync(cmd, args, {
              cwd: tmpWorkspace,
              env: {
                ...process.env,
